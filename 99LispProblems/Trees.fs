...
//Irrelevant since we can define a tree DataType 
...
let rec bincbal num = if num = 2 then Node("x",Node("x",Empty,Empty),Empty) 
                              else let left = bincbal ((num - 1) /2)
                                   let right = bincbal ((num - 1) % 2 + (num - 1) /2)
                                   Node("x" ,left, right)

let res55 = bincbal 5 


let rec bincbalcont num cont = if num = 2 then cont (Node("x",Node("x",Empty,Empty),Empty))
                                          else bincbalcont ((num - 1) /2) (fun leftpart -> 
                                               bincbalcont ((num - 1) % 2 + (num - 1) /2) (fun rightpart -> 
                                               Node("x" ,leftpart, rightpart)))
                                      
let res55c = bincbalcont 5 (fun r -> r)                            

//same but using a continuation 
let rec cbalcont num cont = if num = 2 then cont (Node("x",[Node("x",[])]))
                                      else cbalcont ((num - 1) /2) (fun leftpart -> 
                                           cbalcont ((num - 1) % 2 + (num - 1) /2) (fun rightpart -> 
                                           Node("x" ,[leftpart; rightpart])))
                                      
                             
let res55b = cbalcont 5 (fun r -> r)
...
// one tree is the mirror image of another. We are only interested in the structure, not in the contents of
// the nodes.

let getValue node = node |> (fun (a, b, c )-> a) 
let isEmpty node =  match node with
                    | Empty -> true 
                    | Node (a,b,c) -> false
    

let rec compareNode node1 node2 = match node1, node2 with 
                                  | Empty,Empty -> true
                                  | Empty, Node(_,_,_) -> false
                                  | Node(_,_,_), Empty -> false 
                                  | Node (d,left,right) , Node(d2, left2, right2) -> (compareNode left left2) && (compareNode right right2)
                              
let bt1 = Node ("x",Node("y",Empty,Node("x",Empty,Empty)),Node("y",Empty,Empty))

let isMirror tree = match tree with 
                    | Empty -> true
                    | Node (d,left,right) -> compareNode left right

let res56 = isMirror bt1


let rec compareNodecont node1 node2 cont = match node1, node2 with 
                                              | Empty,Empty -> cont true
                                              | Empty, Node(_,_,_) -> cont false
                                              | Node(_,_,_), Empty -> cont false
                                              | Node (d,left,right) , Node(d2, left2, right2) -> compareNodecont left left2 (fun leftResult -> 
                                                                                                        (compareNodecont right right2 (fun rightResult ->
                                                                                                                leftResult && rightResult)))
let isMirrorCont tree = match tree with 
                        | Empty -> true
                        | Node (d,left,right) -> compareNodecont left right (fun r-> r)

let res56b = isMirrorCont bt1
...
let rec performAddNode tree i = match tree with 
                                | Empty -> Node(i,Empty,Empty)
                                | Node (x,a,b) -> if i < x then 
                                                    Node(x, performAddNode a i,b)
                                                  else 
                                                    Node(x, a,performAddNode b i)
let addNode tree i = performAddNode tree i

let construct list = List.fold addNode Empty list 


let res57 = construct [5;2;4;10;1;4]
